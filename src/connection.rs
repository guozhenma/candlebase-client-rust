...
use crate::protocol::{decode, encode, get_round_ref, set_round_ref, ProtocolMsg, SendError};
use actix::{
    prelude::{Request, *},
...
impl RoundCompleter {
    fn new(round_ref: u32, connection_inner: Rc<ConnectionInner>) -> Self {
        connection_inner.round_attachments.borrow_mut().insert(
            round_ref,
            RoundAttachment {
                response: None,
                waker: None,
...
impl ConnectionInner {
    ...
    pub async fn send(self: Rc<Self>, mut msg: ProtocolMsg) -> Result<ProtocolMsg, SendError> {
        ...
        let msg = set_round_ref(&mut msg, round_ref);
                ...
                match res {
                    Ok(frame) => match frame {
                        Frame::Binary(bytes) => {
                            ...
                            let round_ref = get_round_ref(&response);
...
impl Connection {
    ...
    pub fn new(endpoint: String) -> Self {
        Connection {
            inner: Rc::new(ConnectionInner::new(endpoint)),
        }
    }

    #[inline]
    pub fn start2(endpoint: String) -> Addr<Self> {
        Connection::start_in_arbiter(&ArbiterPool::singleton().fetch_arbiter(), move |_ctx| {
            Connection::new(endpoint)
        })
    }

    pub fn start3(endpoint: String, arbiter: ArbiterHandle) -> Addr<Self> {
...
impl Actor for Connection {
    ...
    fn started(&mut self, ctx: &mut Self::Context) {
        ...
        ctx.spawn(Box::pin(
            Rc::clone(&self.inner).connect_repeatedly().into_actor(self),
        ));
        ctx.spawn(Box::pin(
            Rc::clone(&self.inner).receive_repeatedly().into_actor(self),
        ));
    }
...
impl Handler<SubscribeConnectionStatusMsg> for Connection {
    ...
    fn handle(
        &mut self,
        msg: SubscribeConnectionStatusMsg,
...
impl Handler<UnsubscribeConnectionStatusMsg> for Connection {
    ...
    fn handle(
        &mut self,
        msg: UnsubscribeConnectionStatusMsg,
        _ctx: &mut Context<Self>,
    ) -> Self::Result {
...
mod tests {
    ...
    use crate::protocol::{IntoEnum, PingReq};
    use actix::prelude::*;
    ...
    async fn test_send_msg() {
        ...
        for _ in 1..2 {
            let msg = PingReq { r#round_ref: 1 }.into_enum();
            let res = conn
                .send(msg)
                .timeout_ext(Duration::from_millis(1000))
                .await;
            println!("result: {:?}", res);
        }
    }
